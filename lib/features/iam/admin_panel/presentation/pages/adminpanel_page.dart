...
class _AdminPanelPageState extends State<AdminPanelPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (_) => AdminPanelBloc(repository: repo),
      child: Builder(
        builder: (ctx) {
          if (!_loadedOnce) {
            WidgetsBinding.instance.addPostFrameCallback((_) async {
              ...
              if (!mounted) return;
              ctx.read<AdminPanelBloc>().add(LoadSubUsers('All', accountId: accountId));
            });
            _loadedOnce = true;
          }
                          ...
                          Widget navItem(IconData icon, String label, String key) {
                            ...
                            return InkWell(
                              ...
                              onTap: () async {
                                final accountId = await TokenStorage().readAccountId();
                                if (!mounted) return;
                                context.read<AdminPanelBloc>().add(LoadSubUsers(key, accountId: accountId));
                              },
                              child: SizedBox(
                                width: 100,
                                child: Column(
